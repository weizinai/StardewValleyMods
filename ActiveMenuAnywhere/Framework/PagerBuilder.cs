...
/// 与原版 AMAMenu 每个页签的内容形态（3×3 九宫格、每页 9 个）保持一致。
/// </summary>
internal static class PagerBuilder
{
    ...
    /// <summary>构造一个分页内容：选项按固定顺序装入多页 3 列网格。</summary>
    /// <param name="options">该页签的选项集合。</param>
    /// <param name="isFavoriteTab">是否收藏页签（决定格子点击的收藏分支）。</param>
    /// <returns>分页内容。</returns>
    public static Pager Build(IReadOnlyList<BaseOption> options, bool isFavoriteTab)
    {
        var pager = new Pager(spacing: PagerSpacing);
        pager.PreviousButton.Text = I18n.UI_Page_Previous();
        pager.NextButton.Text = I18n.UI_Page_Next();
