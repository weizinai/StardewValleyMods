...
internal class ModLimitHandler : BaseHandler
{
    ...
    public ModLimitHandler(IModHelper helper, ModConfig config)
        : base(helper, config)
    {
        this.modRequirement = helper.Data.ReadJsonFile<Dictionary<string, string[]>>(ModRequirementPatch);
        if (this.modRequirement is null)
    ...
    {
        // 如果功能未启用，则返回
        if (!this.Config.ModLimit) return;
        ...
        var unAllowedMods = this.GetUnAllowedMods(e.Peer).ToList();
    ...
    private void OnModMessageReceived(object? sender, ModMessageReceivedEventArgs e)
    {
        ...
        if (e is { Type: "ModLimit", FromModID: "weizinai.SomeMultiplayerFeature" })
        {
            ...
            foreach (var id in message) Log.Info(this.modRequirement!["RequiredModList"].Contains(id) ? $"{id}未安装" : $"{id}被禁止");
        }
    }
    ...
    private IEnumerable<string> GetUnAllowedMods(IMultiplayerPeer peer)
    {
        ...
        foreach (var id in this.modRequirement!["RequiredModList"])
        {
        ...
        {
            if (!this.modRequirement["RequiredModList"].Contains(id) && !this.modRequirement["AllowedModList"].Contains(id))
            {
                yield return id;
            }
        }
